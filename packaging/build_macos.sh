...
echo "          (or: ./scripts/launch-macos.sh — strips Gatekeeper flags)"
echo ""
echo "  For distribution with a paid Apple Developer ID:"
echo "    # 1) Sign with your Developer ID (sign nested deps + the bundle)"
echo "    codesign --deep --force --options runtime \\"
echo "             --sign \"Developer ID Application: Your Name (TEAMID)\" \\"
echo "             dist/SmartBI.app"
echo "    # 2) Zip and submit for notarization"
echo "    ditto -c -k --sequesterRsrc --keepParent dist/SmartBI.app dist/SmartBI.app.zip"
echo "    xcrun notarytool submit dist/SmartBI.app.zip \\"
echo "             --keychain-profile <profile> --wait"
echo "    # 3) Staple the notarization ticket so the .app is verifiable offline"
echo "    xcrun stapler staple dist/SmartBI.app"
